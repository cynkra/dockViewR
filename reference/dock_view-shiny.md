# Shiny bindings for dock_view

Output and render functions for using dock_view within Shiny
applications and interactive Rmd documents.

## Usage

``` r
dockViewOutput(outputId, width = "100%", height = "400px")

dock_view_output(outputId, width = "100%", height = "400px")

renderDockView(expr, env = parent.frame(), quoted = FALSE)

render_dock_view(expr, env = parent.frame(), quoted = FALSE)
```

## Arguments

- outputId:

  output variable to read from

- width, height:

  Must be a valid CSS unit (like `'100%'`, `'400px'`, `'auto'`) or a
  number, which will be coerced to a string and have `'px'` appended.

- expr:

  An expression that generates a dock_view

- env:

  The environment in which to evaluate `expr`.

- quoted:

  Is `expr` a quoted expression (with
  [`quote()`](https://rdrr.io/r/base/substitute.html))? This is useful
  if you want to save an expression in a variable.

## Value

`dockViewOutput` and `dock_view_output` return a Shiny output function
that can be used in the UI definition. `renderDockView` and
`render_dock_view` return a Shiny render function that can be used in
the server definition to render a `dock_view` element.
